...
def main(args):
    ...
    x_edges = tf.nn.conv2d(X_, W, strides=[1,1,1,1], padding='SAME')
    y_edges = tf.nn.conv2d(output, W, strides=[1,1,1,1], padding='SAME')

    trans_loss = tf.nn.l2_loss(output-Y)
    feat_loss = tf.reduce_sum(tf.abs(x_edges - y_edges))
