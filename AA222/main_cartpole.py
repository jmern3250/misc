...
def VPG(env, graph, sess, X, Y, n_steps):
	...
	A = tf.placeholder(dtype=tf.float32, shape=[None,2], name='A')
	...
	while n < n_steps:
		...
		while not done:
			...
			action = np.random.choice([0,1], p=p[0])
...
def train(env, num_timesteps, seed):
	...
	saver.save(sess, './models/cartpole/sgd_model')
...
def main(args):
	...
	# with open('./models/cartpole/SGD/mlp0.p', 'wb') as f: 
	# 	pickle.dump(vars_list, f)
	# sess.close()

	# #### GA Fine Tuning #### 	
	# with open('./models/cartpole/SGD/mlp0.p', 'rb') as f: 
	# 	vars_list = pickle.load(f)

	# env = CPS()
	# graph, X, Y, _, sess = build_graph(vars_list)

	# eval_dict = {'env':env, 'n_episodes':15, 'graph':graph, 
	# 			 'X':X, 'Y':Y, 'sess':sess}
	# results = genetic_optimizer.main([vars_list], evaluation, 
	# 	eval_fcn_arg_dict=eval_dict,
	# 	n_itrs=10, population_size=50, n_survivors=10, 
	# 	p_crossover=0.5, mutation_std=0.1, noise_decay=0.9)
	# with open('./models/cartpole/GA/results0.p', 'wb') as f: 
	...
	# with open('./models/cartpole/SGD/mlp1.p', 'wb') as f: 
	# 	pickle.dump(vars_list, f)

	#### Eval GA ####
	with open('./models/cartpole/GA/results0.p', 'rb') as f: 
	...
	#### Eval SGD ####
	with open('./models/cartpole/SGD/mlp1.p', 'rb') as f: 
		vars_list = pickle.load(f)
	graph, X, Y, _, sess = build_graph(vars_list)
